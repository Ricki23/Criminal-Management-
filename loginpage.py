    ...
    if username == "admin" and password == "password":
        messagebox.showinfo("Login Successful", "Welcome, " + username + "!")
            ...
            def __init__(abc, root):
                ...
                no = Radiobutton(radio_frame_wanted, variable=abc.var_wanted, text='No', value='no',
                                 font=('arial', 9, 'bold'),
                                 bg='white')
                ...
                btn_add = Button(button_frame, text="Record Save", command=abc.add_data, font=("arial", 13, "bold"),
                                 width=14,
                                 bg='blue', fg='white')
                btn_add.grid(row=0, column=0, padx=3, pady=5)

                btn_update = Button(button_frame, text="Update", command=abc.update_data, font=("arial", 13, "bold"),
                                    width=14,
                                    bg='blue', fg='white')
                btn_update.grid(row=0, column=1, padx=1, pady=5)

                btn_delete = Button(button_frame, text="Delete", command=abc.delete_data, font=("arial", 13, "bold"),
                                    width=14,
                                    bg='blue', fg='white')
                btn_delete.grid(row=0, column=2, padx=1, pady=5)

                btn_clear = Button(button_frame, text="Clear", command=abc.reset_data, font=("arial", 13, "bold"),
                                   width=14,
                                   bg='blue', fg='white')
                ...
                stayhome = Label(search_frame, text="NATIONAL CRIME AGENCY", font=("times new roman", 30, "bold"),
                                 fg="crimson",
                                 bg="white")
            ...
            def serach_data(abc):
                if abc.var_com_search.get() == '' or abc.var_search.get() == '':
                    ...
                    try:
                        ...
                        my_cursor = conn.cursor()
                        my_cursor.execute(
                            'select * from criminal1 where ' + str(abc.var_com_search.get()) + " LIKE'%" + str(
                                abc.var_search.get() + "%'"))
