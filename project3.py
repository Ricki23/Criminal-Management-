...
class Criminal:
    def __init__(self, root):
        ...
        lbl_title = Label(self.root, text="CRIMINAL MANAGEMENT SYSTEM SOFTWARE", font=("times new roman", 40, "bold"),
                          bg="white", fg="blue")
        lbl_title.place(x=0, y=0, width=1300, height=70)


        img_frame=Frame(self.root,bd=2,relief=RIDGE,bg='white')
        img_frame.place(x=0,y=80,width=1550,height=200)

        #1st
        img_logo = Image.open('police1.jpg')
        img_logo = img_logo.resize((490, 160), Image.ANTIALIAS)
        self.photo_logo = ImageTk.PhotoImage(img_logo)

        self.logo = Label(img_frame, image=self.photo_logo)
        self.logo.place(x=0, y=0, width=490, height=160)

        #2nd
        img_logo2 = Image.open('police2.jpg')
        img_logo2 = img_logo2.resize((490, 160), Image.ANTIALIAS)
        self.photo_logo2 = ImageTk.PhotoImage(img_logo2)

        self.logo2 = Label(img_frame, image=self.photo_logo2)
        self.logo2.place(x=500, y=0, width=490, height=160)

        #3rd

        img_logo3 = Image.open('police3.jpg')
        img_logo3 = img_logo3.resize((490, 160), Image.ANTIALIAS)
        self.photo_logo3 = ImageTk.PhotoImage(img_logo3)

        self.logo3 = Label(img_frame, image=self.photo_logo3)
        self.logo3.place(x=1000, y=0, width=280, height=160)
        ...
        img_logo4 = Image.open('criminal1.jpg')
        img_logo4 = img_logo4.resize((220, 220), Image.ANTIALIAS)
        self.photo_logo4 = ImageTk.PhotoImage(img_logo4)

        self.logo4 = Label(upper_frame, image=self.photo_logo4)
        self.logo4.place(x=1000, y=0, width=220, height=220)
        ...
        if self.var_com_search.get() == '' or self.var_search.get() == '':
            ...
            try:
                conn = mysql.connector.connect(host='localhost', user='root', password='root',
                                               database='management')
                my_cursor = conn.cursor()
                my_cursor.execute('select * from criminal1 where ' + str(self.var_com_search.get()) + " LIKE'%" + str(
                    self.var_search.get() + "%'"))
